...
We will be using `dplyr` package for data clening and manipulation.
...
```{R}
length(unique(stormData$eventType[grepl("thun.*orm", stormData$eventType, ignore.case = TRUE)]))
...
topHumanDamage <- arrange(allDamage,desc(sumHumanImpact))  %>% select(eventType,sumHumanImpact) %>% top_n(12)
```
#### Top Human Damage
```{R}
kable(topHumanDamage)
```
...
Zoom into storm data to focus on economic damage less than $100 billion and 12,500 human lives affected.
